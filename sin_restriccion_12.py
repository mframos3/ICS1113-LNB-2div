...
for arb in arbitros:
    for fecha in fechas:
        if visita!=local:
            modelo.addConstr((quicksum(quicksum(arbitrar_aijt[arb,local,visita,fecha] for local in equipos) for visita in equipos)) <= 1)

# R9
for local in equipos:
    for fecha in fechas:
        if local != visita:
            modelo.addConstr(quicksum(quicksum(arbitrar_aijt[arb,local,visita,fecha] + arbitrar_aijt[arb,visita,local,fecha] for arb in arbitros) for visita in equipos)
        ...
        if visita != local:
            for t in fechas[1:len(fechas)//2]:
                for h in fechas[(len(fechas)//2)+1:]:
                    modelo.addConstr(partido_ijt[local,visita,t] + partido_ijt[visita,local,t] +partido_ijt[local,visita,t-1] + partido_ijt[visita,local,t-1] +
                    partido_ijt[local, visita, h] + partido_ijt[visita, local, h] + partido_ijt[local, visita, h - 1] + 
                    partido_ijt[visita, local, h - 1] - incumple_itn[local,t,1] <=3)
...
for local in equipos:
    for equipok in equipos:
        for visita in equipos:
            if local != visita and local != equipok:
    ...
    for arb in arbitros:
        for fecha1 in range(2, (len(fechas) + 1)):
            for fecha2 in range(1, fecha1):
                    modelo.addConstr(quicksum(partido_ijt[local,j,fecha1] + partido_ijt[j,local,fecha1] + arbitrar_aijt[arb,local,j,fecha1] + arbitrar_aijt[arb,j,local,fecha1] for j in equipos if j!=local)
                                     + quicksum(partido_ijt[local,l,fecha2] + partido_ijt[l,local,fecha2] + arbitrar_aijt[arb,local,l,fecha2] + arbitrar_aijt[arb,l,local,fecha2] for l in equipos if l!=local)
                                     - incumple_itn[local,fecha1,4] <= 3)

# R16
for n in cons:
    for n in cons:
        modelo.addConstr((quicksum(quicksum(incumple_itn[equipo, fecha, n] for equipo in equipos) for fecha in fechas)) == incumple_n[n])
